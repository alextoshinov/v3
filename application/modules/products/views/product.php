	...
	<?php if(count($options) > 0): ?>
                <?php foreach($options as $option):
                    ...
                    ?>
                        ...
                        <?php elseif($option->type == 'colorlist'):?>
                            <ul class="swatches Color colorLi">
                            <?php foreach ($option->values as $values):?>
                            <?php $checked = '';
                                if($value == $values->id)
                                {
                                ...
                                <input style="visibility:hidden;" class="radioColor" <?php echo $checked;?> type="radio" name="option[<?php echo $option->id;?>]" value="<?php echo $values->id;?>"/>
                                  
                            <li class="" data-color="<?php echo $values->color;?>"> 
                                <a class="colorCheckLink" data-option-id="<?php echo $option->id;?>" data-values-id="<?php echo $values->id;?>" title="<?php echo $values->name;?>" style="background-color:<?php echo $values->color;?>"> </a>
                            </li>                                
                            <?php endforeach;?>    
                            </ul>                            
                        <?php elseif($option->type == 'sizelist'):?> 
                            <ul class="swatches Color sizeLi">
                            <?php foreach ($option->values as $values):?>
                            <?php $checked = '';
                                if($value == $values->id)
                                {
                                ...
                                <input style="visibility:hidden;" <?php echo $checked;?> type="radio" name="option[<?php echo $option->id;?>]" value="<?php echo $values->id;?>"/>
  
                                <li class="" data-color="<?php echo $values->name;?>"> 
                                    <a class="sizeCheckLink" data-option-id="<?php echo $option->id;?>" data-values-id="<?php echo $values->id;?>" title="<?php echo $values->name;?>" style="background-color:#999999;color:#FFFFFF"><?php echo $values->name;?></a>
                                </li>                             
                            <?php endforeach;?>    
                            </ul>                           
...
<!--							<button type="button" title="Wishlist" class="btn btn-wishlist">
	...
	<!-- Product Info Ends -->
        <?php //include(__DIR__.'/product_tabs.php');?>
        <?php if(!empty($product->related_products)):?>
        <!-- Related Products Starts -->
        <div class="product-info-box">
            <h4 class="heading"><?php echo lang('related_products_title');?></h4>
        <?php
        $relatedProducts = [];
...
//        print '<pre>'; print_r($relatedProducts);exit;
...
<script>
    ...
    $(document).ready(function(){
        ...
        //
        $("body").on("click", ".colorCheckLink", function () {
            var optionId = $(this).attr("data-option-id");
            var valueId = $(this).attr("data-values-id");
            console.log('OptionId: ',optionId);
            console.log('Value Id: ',valueId);
            $(".colorLi li").removeClass('selected');
            $(this).parent().addClass('selected');
            $('input[name="option[' + optionId + ']"][value="' + valueId + '"]').prop('checked', true);
        });
        //
        $("body").on("click", ".sizeCheckLink", function () {
            var optionId = $(this).attr("data-option-id");
            var valueId = $(this).attr("data-values-id");
            console.log('OptionId: ',optionId);
            console.log('Value Id: ',valueId);
            $(".sizeLi li").removeClass('selected');
            $(this).parent().addClass('selected');
            $('input[name="option[' + optionId + ']"][value="' + valueId + '"]').prop('checked', true);
        });
        //
